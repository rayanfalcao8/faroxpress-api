...
class TransferPaymentController extends Controller
{
    ...
    public function markPaid(Request $request, Transfer $transfer): JsonResponse
    {
        ...
        try {
            if ($transfer->status === TransferStatus::PENDING_PAYMENT || $transfer->status === TransferStatus::CREATED) {
                if ($transfer->status === TransferStatus::CREATED) {
                    $this->statusMachine->transition($transfer, TransferStatus::PENDING_PAYMENT);
                }

                $this->statusMachine->transition($transfer, TransferStatus::PAID);
                $transfer->paid_at = now();

                $this->statusMachine->transition($transfer, TransferStatus::PROCESSING);
                ...
                if ($result['ok']) {
                    $this->statusMachine->transition($transfer, TransferStatus::COMPLETED);
                    $transfer->processed_at = now();
                    $transfer->failure_reason = null;
                } else {
                    $this->statusMachine->transition($transfer, TransferStatus::FAILED);
